...
const validateCred = (ccNumber) => {
	const reversed = ccNumber.reverse();
	let sum = 0;
	for (let i = 1; i < reversed.length; i += 2) {
		let newNum = reversed[i] * 2;
		if (newNum > 9) {
			newNum -= 9;
		}
		sum += newNum;
...
const findInvalidCards = (ccNumber) => {
	let invalidCards = [];
	ccNumber.forEach((element) => {
...
const idInvalidCardCompanies = (invalidNumbers) => {
	...
	invalidCompanies = invalidCompanies.filter(function (item, pos, self) {
